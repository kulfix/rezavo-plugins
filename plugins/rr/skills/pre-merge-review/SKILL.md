...
### Task 5: Round 2 Decision

**Skip R2 if:** R1 had 0 MUST FIX AND 0 DEFERRED → go to Task 8.

**Run R2 if:** R1 had ANY MUST FIX or DEFERRED.

### Task 6: Round 2 — Audit (regression check)

Invoke `rr:audit` with ONLY Fletcher + Paranoik (2 agents). Focus: verify R1 fixes didn't introduce regressions.

### Task 7: Round 2 — Fix + Commit + Report

Same as Tasks 3-4 but for Round 2:
...
| Mistake | Fix |
|---------|-----|
| Running git diff yourself | Audit handles scope. Task 1 is directory setup only. |
